...
void token_operators(char curr, char next, FILE *file)
{
    ...
    // Only consume the second char if it was actually part of a two-character operator
    if (twoCharOpUsed)
    {
        fgetc(file); // consume the second char
    }
}
   ...
   if (*in_string)
{
    ...
    if (curr == '\'') 
    {
        ...
        if (next_char == '\\') {
            ...
            // Normal character
            if (next_char == EOF || fgetc(file) != '\'') {
                return; // malformed
            }
            char_value[0] = next_char;
            char_value[1] = '\0';
        }
...
// Handle '.' — could be part of a real number or standalone T_DOT
if (curr == '.') {
    char next_next = fgetc(file);
    ungetc(next_next, file);

    if (isdigit(next_next)) {
        // It's part of a real number like .14
        if (!(*is_formation)) {
            *index = 0;
        }
        *is_formation = 1;
        ...
        // It's a standalone dot operator
        if (*is_formation) {
            formation[*index] = '\0';
            Token_Formation(formation);
            *is_formation = 0;
...
//- or + only if previous char was E or e
if ((curr == '+' || curr == '-') && *index > 0 &&
    (formation[*index - 1] == 'E' || formation[*index - 1] == 'e')) {
    if (*index < MaxTokenLen - 1) {
        formation[(*index)++] = curr;
        return;
    }
